 ...
 */
export function Square({ value, squareState, onSquareClick }: SquareProps) {
  // Colour that square backgrounds will be set to whether they're dug or not dug
  const dugSquareColour: string = "#cfcfcf";
  const notDugSquareColour: string = "#f0f0f0";
  let valueColour: string = "black";

  /**
   * Returns a specific colour from a given number.
   * Default colour returned is "black".
   */
  function getValueColour(someValue: number) {
    let valueColour;
    switch (someValue) {
      case 1:
        valueColour = "blue";
